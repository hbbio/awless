	...
	if err := dec.Decode(&latest); err == nil {
		if IsSemverUpgrade(Version, latest.Version) {
			...
			case "zip", "targz":
				...
				install = fmt.Sprintf("Run `wget -O awless-%s.%s https://github.com/hbbio/awless/releases/download/%s/awless-%s-%s.%s`", latest.Version, ext, latest.Version, runtime.GOOS, runtime.GOARCH, ext)
			default:
				install = "Run `go get -u github.com/hbbio/awless`"
			}
			fmt.Fprintf(messaging, "New version %s available. Checkout the latest features at https://github.com/hbbio/awless/blob/master/CHANGELOG.md\n%s\n", latest.Version, install)
		}
	}
